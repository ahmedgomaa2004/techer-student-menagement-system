...
const updateRoleToggle = () => {
  ...
  if (btn) {
    btn.textContent = state.role === "teacher" ? "المعلم" : "الطالب";
  }
  if (badge) {
    badge.textContent = state.role === "teacher" ? "وضع المعلم" : "وضع الطالب";
  }
};
...
const renderDashboard = () => {
  ...
  const openAssignments = assignments.filter((a) => a.status !== "مغلق").length;
  const paidCount = payments.filter((p) => p.status === "مدفوع").length;
      ...
      <div class="col-md-4">
        <div class="glass-card h-100">
          ...
          <div class="mt-3 d-flex justify-content-between align-items-center">
            <span class="badge badge-neon">تقييم ${t.rating}</span>
            <button class="btn btn-outline-primary btn-sm" data-teacher-id="${t.id}" data-bs-toggle="modal" data-bs-target="#teacherModal">التفاصيل</button>
          </div>
        </div>
      </div>
    `
  ...
  grid.addEventListener("click", (event) => {
    ...
    document.getElementById("teacherModalBody").innerHTML = `
      <div class="d-flex flex-column gap-2">
        <div><strong>المادة:</strong> ${teacher.subject}</div>
        <div><strong>البريد:</strong> ${teacher.email}</div>
        <div><strong>الهاتف:</strong> ${teacher.phone}</div>
        <div><strong>التقييم:</strong> ${teacher.rating}</div>
      </div>
    `;
  });
};
    ...
    .map(
      ...
      <tr>
        ...
        <td data-role="teacher">
          <button class="btn btn-outline-primary btn-sm" data-edit-student="${student.id}" data-bs-toggle="modal" data-bs-target="#studentModal" data-role="teacher">تعديل</button>
        </td>
      </tr>
    `
    )
...
const initStudentsInteractions = () => {
  ...
  if (addBtn) {
    addBtn.addEventListener("click", () => {
      ...
      document.getElementById("studentClass").value = "أ/1";
      document.getElementById("studentLevel").value = "العاشر";
        ...
        <div class="col-md-4">
          <div class="glass-card h-100">
            ...
            <div class="mt-3 d-flex justify-content-between">
              <span class="badge badge-purple">${count} طلاب</span>
              <a class="btn btn-outline-primary btn-sm" href="class-details.html?classId=${cls.id}">عرض التفاصيل</a>
            </div>
          </div>
        </div>
      `;
...
const renderAttendanceTable = () => {
  ...
  tbody.innerHTML = window.appData.attendance
    .map((record) => {
      ...
      const badgeClass = record.status === "حاضر" ? "badge-neon" : "badge-magenta";
      return `
      <tr>
        <td>${student.name}</td>
        <td>${record.date}</td>
        <td><span class="badge ${badgeClass}">${record.status}</span></td>
        <td><button class="btn btn-outline-primary btn-sm" data-attendance-id="${record.id}">تبديل</button></td>
      </tr>
    `;
    })
    .join("");
...
const renderGradesTable = () => {
  ...
  if (data.length === 0) {
    tbody.innerHTML = `
      <tr>
        <td colspan="5">
          <div class="empty-state">لا توجد درجات بعد</div>
        </td>
      </tr>
    `;
    return;
    ...
    .map((grade) => {
      ...
      return `
        <tr>
          ...
          <td>${grade.score}</td>
          <td>${grade.date}</td>
          <td><span class="badge badge-neon">${grade.score >= 85 ? "ممتاز" : "جيد"}</span></td>
        </tr>
      `;
    })
    .join("");
    ...
    .map(
      ...
      <div class="col-md-4">
        <div class="glass-card h-100">
          ...
          <div class="mt-2 small">موعد التسليم: ${a.dueDate}</div>
          <div class="mt-3 d-flex justify-content-between align-items-center">
            <span class="badge ${a.status === "مغلق" ? "badge-magenta" : "badge-neon"}">${a.status}</span>
            <button class="btn btn-outline-primary btn-sm" data-bs-toggle="modal" data-bs-target="#assignmentModal">رفع/تسليم</button>
          </div>
        </div>
      </div>
    `
...
const renderPaymentsTable = () => {
  ...
  tbody.innerHTML = data
    .map((payment) => {
      ...
      const badgeClass = payment.status === "مدفوع" ? "badge-neon" : "badge-magenta";
      return `
      <tr>
        <td>${student.name}</td>
        ...
        <td>
          <button class="btn btn-primary btn-sm" data-pay-id="${payment.id}" data-role="student">ادفع الآن</button>
        </td>
      </tr>
    `;
    })
...
const renderProfile = () => {
  ...
  document.getElementById("profileRole").value = isTeacher ? "معلم" : "طالب";
};
